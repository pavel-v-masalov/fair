...
select /*+ PARALLEL(4) */ r.opportunityname, r.opportunityid, r.countedon,
    ...
    /*MIN(isPrescorig) KEEP (DENSE_RANK FIRST ORDER BY isPrescorig nulls last)*/cast(null as varchar2(100)) isPrescorig,--Ğàñ÷¸ò â ğàìêàõ ïğåñêîğèíãà
    /*MIN(MODEL_TYPE) KEEP (DENSE_RANK FIRST ORDER BY MODEL_TYPE nulls last)*/cast(null as varchar2(100)) MODEL_TYPE,--Òèï ìîäåëè
    /*MIN(calc_conditions) KEEP (DENSE_RANK FIRST ORDER BY calc_conditions nulls last)*/cast(null as varchar2(100)) calc_conditions,--Óñëîâèå ïîïàäàíèÿ â ïîâòîğíûå
    /*MIN(ISPRESCORINGCALL) KEEP (DENSE_RANK FIRST ORDER BY ISPRESCORINGCALL nulls last)*/cast(null as varchar2(100)) ISPRESCORINGCALL,--Ğàñ÷¸ò â ğàìêàõ ïğåñêîğèíãà
    /*MIN(model_settings) KEEP (DENSE_RANK FIRST ORDER BY model_settings nulls last)*/ cast(null as varchar2(100)) model_settings--Êîğòåæ âêëş÷åííûõ ìîäåëåé íà ììîåíò ğàñ÷¸òà
  from DWH.RMD r
